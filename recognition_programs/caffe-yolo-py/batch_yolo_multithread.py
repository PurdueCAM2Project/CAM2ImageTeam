...
#		ftxt = open(self.tofile_txt,'w')
	for i in range(len(results)):
		...
		if disp_console : print '    class : ' + results[i][0] + ' , [x,y,w,h]=[' + str(x-aw) + ',' + str(y-ah) + ',' + str(int(results[i][3])) + ',' + str(int(results[i][4]))+'], Confidence = ' + str(results[i][5])
		xmin = x-w
		xmax = x+w
		ymin = y-h
		ymax = y+h
...
def resize_and_process(line, img):
	print '\nStarting thread for {0} at time {1}'.format(line, str(time.ctime(time.time())))
	inputs = img
	start = datetime.now()
	...
	print 'FILE: {0} Time spent: {1} sec'.format(line, elapsedTime)
	#print out.iteritems()
...
def main(argv):
	...
	try:
		...
		print 'yolo_main.py -m <model_file> -w <output_file> -i <img_list_file>'
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'yolo_main.py -m <model_file> -w <weight_file> -i <img_list_file>'
			sys.exit()
		elif opt == "-m":
			model_filename = arg
		elif opt == "-w":
			...
			img_list_filename = arg
	print 'model file is "', model_filename
	print 'weight file is "', weight_filename
	print 'image list file is "', img_list_filename
	global q
	q = Queue()
	global net
	...
	for line in img_list_file:
		...
		try:
			thread.start_new_thread(resize_and_process, (line, img))
		except:
   			print "Error: unable to start thread"
		
		imagesProcessed+=1
		#cv2.waitKey(10000)
	endOverall = datetime.now()

	time_spent=(endOverall-startOverall).total_seconds()
	print 'In total, {0} FPS'.format(imagesProcessed/time_spent)
